...
export const useCartB2B = () => {
  ...
  const onAdd = (product, quantity, shopName) => {
  ...
  const toggleCartItemQuanitity = (id, value) => {
    ...
    if(value === 'inc') {
        cartItems.splice(index, 1, {
            ...
            subtotal: foundItem?.quantity * foundItem?.price
          });
  ...
  const quoteCartlculator = (quoteCartItems) => {
    const quoteProducts = JSON.parse(quoteCartItems?.cart_items)
...
console.log('==================lllllll===', quoteProducts, calculatedSubtotal, quoteQuantity)
    return {
  ...
  // TODO: Implement a situation when that prevents user from adding quote with a different shopId. Quotes are only associated to 1 shop. so products from another store cannot be addded to a sifferent store.

  // TOD: check if user has an unsubmitted quote in the cart.

  // TODO: Notifications for incomplete quotes.

  // TODO: ability to edit quote from quote request page ...
