...
const ModalWishlist = ({ stopPropagation }) => {
  

  const {modalType, closeModal} = useGlobalState()

  return (
    <div onClick={closeModal} className={`${modalType==='wishlist' ? 'opacity-100 scale-y-100' : 'opacity-50 scale-y-0' } transition-transform transform duration-500 absolute inset-0  z-50`}
